...
TEST(UserManagerTest, test_user_manager_get_events)
{
	...
	ASSERT_TRUE(user_manager.get_events("zmkeil", 1, 10, &db_helper, &records, NULL));
...
TEST(UserManagerTest, test_user_manager_set_events)
{
	microbill::UserManager user_manager;
	ASSERT_TRUE(user_manager.init(env->microbill_config.user_options()));
	...
	ASSERT_FALSE(user_manager.set_events("zz", records, NULL));
	ASSERT_TRUE(user_manager.set_events("zmkeil", records, NULL));
}
